...
import { scienceQuestions } from '@/utils/Questions';
...
const Page = () => {
    ...
    const calculateScore = () => {
        const questions = scienceQuestions[selectedLevel];
        let score = 0;
    ...
    const allQuestionsAnswered = selectedAnswers.length === scienceQuestions[selectedLevel].length;
  
    return (
        <div className='mt-10 f'>
            <Heading level={2} className='text-center'>Science Quiz</Heading>
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                {selectedLevel === "easy" && (
                    scienceQuestions.easy.map((question, index) => (
                 ...
                 {selectedLevel === "medium" && (
                       scienceQuestions.medium.map((question, index) => (
                            <div key={index} className='text-white'>
                                <Paragraph className='mt-4'>{question.question}</Paragraph>
                                <ul className='mt-1'>
                                    {question.options.map((option, optionIndex) => (
                ...
                {selectedLevel === "difficult" && (
                      scienceQuestions.difficult.map((question, index) => (
                        <div key={index} className='text-white'>
                            <Paragraph className='mt-4'>{question.question}</Paragraph>
                            <ul className='mt-1'>
                                {question.options.map((option, optionIndex) => (
