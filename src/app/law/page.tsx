...
import { lawQuestions } from '@/utils/Questions';
...
const Page = () => {
    ...
    const calculateScore = () => {
        const questions = lawQuestions[selectedLevel];
        let score = 0;
    ...
    const allQuestionsAnswered = selectedAnswers.length === lawQuestions[selectedLevel].length;
  
    return (
        <div className='mt-10 f'>
            <Heading level={2} className='text-center'>Law Quiz</Heading>
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                {selectedLevel === "easy" && (
                    lawQuestions.easy.map((question, index) => (
                 ...
                 {selectedLevel === "medium" && (
                       lawQuestions.medium.map((question, index) => (
                            <div key={index} className='text-white'>
                                <Paragraph className='mt-4'>{question.question}</Paragraph>
                                <ul className='mt-1'>
                                    {question.options.map((option, optionIndex) => (
                ...
                {selectedLevel === "difficult" && (
                      lawQuestions.difficult.map((question, index) => (
                        <div key={index} className='text-white'>
                            <Paragraph className='mt-4'>{question.question}</Paragraph>
                            <ul className='mt-1'>
                                {question.options.map((option, optionIndex) => (
