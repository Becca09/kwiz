...
import { blockchainQuestions } from '@/utils/Questions';
...
const Page = () => {
    ...
    const calculateScore = () => {
        const questions = blockchainQuestions[selectedLevel];
        let score = 0;
    ...
    const allQuestionsAnswered = selectedAnswers.length === blockchainQuestions[selectedLevel].length;
  
    return (
        <div className='mt-10 f'>
            <Heading level={2} className='text-center'>Blockchain Quiz</Heading>
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                {selectedLevel === "easy" && (
                    blockchainQuestions.easy.map((question, index) => (
                 ...
                 {selectedLevel === "medium" && (
                      blockchainQuestions.medium.map((question, index) => (
                            <div key={index} className='text-white'>
                                <Paragraph className='mt-4'>{question.question}</Paragraph>
                                <ul className='mt-1'>
                                    {question.options.map((option, optionIndex) => (
                ...
                {selectedLevel === "difficult" && (
                      blockchainQuestions.difficult.map((question, index) => (
                        <div key={index} className='text-white'>
                            <Paragraph className='mt-4'>{question.question}</Paragraph>
                            <ul className='mt-1'>
                                {question.options.map((option, optionIndex) => (
