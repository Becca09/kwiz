...
import { mathematicsQuestions, } from '@/utils/Questions';
...
const MathematicsQuizPage = () => {
    ...
    const calculateScore = () => {
        const questions = mathematicsQuestions[selectedLevel];
        let score = 0;
    ...
    const allQuestionsAnswered = selectedAnswers.length === mathematicsQuestions[selectedLevel].length;
  
    return (
        <div className='mt-10 f'>
            <Heading level={2} className='text-center'>Mathematics Quiz</Heading>
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                {selectedLevel === "easy" && (
                    mathematicsQuestions.easy.map((question, index) => (
                 ...
                 {selectedLevel === "medium" && (
                        mathematicsQuestions.medium.map((question, index) => (
                            <div key={index} className='text-white'>
                                <Paragraph className='mt-4'>{question.question}</Paragraph>
                                <ul className='mt-1'>
                                    {question.options.map((option, optionIndex) => (
                ...
                {selectedLevel === "difficult" && (
                       mathematicsQuestions.difficult.map((question, index) => (
                        <div key={index} className='text-white'>
                            <Paragraph className='mt-4'>{question.question}</Paragraph>
                            <ul className='mt-1'>
                                {question.options.map((option, optionIndex) => (
...
export default MathematicsQuizPage;