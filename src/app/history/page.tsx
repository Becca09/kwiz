...
import {historyQuestions } from '@/utils/Questions';
...
const Page = () => {
    ...
    const calculateScore = () => {
        const questions = historyQuestions[selectedLevel];
        let score = 0;
    ...
    const allQuestionsAnswered = selectedAnswers.length === historyQuestions[selectedLevel].length;
  
    return (
        <div className='mt-10 f'>
            <Heading level={2} className='text-center'>History Quiz</Heading>
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                {selectedLevel === "easy" && (
                    historyQuestions.easy.map((question, index) => (
                 ...
                 {selectedLevel === "medium" && (
                       historyQuestions.medium.map((question, index) => (
                            <div key={index} className='text-white'>
                                <Paragraph className='mt-4'>{question.question}</Paragraph>
                                <ul className='mt-1'>
                                    {question.options.map((option, optionIndex) => (
                ...
                {selectedLevel === "difficult" && (
                      historyQuestions.difficult.map((question, index) => (
                        <div key={index} className='text-white'>
                            <Paragraph className='mt-4'>{question.question}</Paragraph>
                            <ul className='mt-1'>
                                {question.options.map((option, optionIndex) => (
